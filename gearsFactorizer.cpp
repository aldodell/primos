...
void gearsFactorizer::find(int64 n) {
  ...
  while (c != n) {
    ciclos++;
    if (debugLevel > 1)
      cout << "Cycle:" << ciclos << " a:" << a << " b:" << b << " a*b:" << c
           << " a-b:" << a - b << endl;
    ...
    if (c == n) {
      if (debugLevel > 0) {
        cout << "Found " << a << " factor. Call recursively to factorize it."
             << endl;
      }
      this->find(a);
      this->find(b);
...
string gearsFactorizer::toString() {
  ...
  for (i = 0; i < this->factors.size(); i++) {
    ...
    for (j = 0; j < this->factorsObj.size(); j++) {
      ...
      // Si existe una representación previamente guardada con la base en
      // cuestión aunmentamos sólo el exponente
      if (obj->base == base) {
        obj->exp++;
        flag = false;
      }
    }
    // Si no exite una representación con la base en cuestión entonces la
    // guardamos
    if (flag) {
  ...
  std::sort(this->factorsObj.begin(), this->factorsObj.end(),
            this->comparefactorObj);
  ...
  for (j = 0; j < this->factorsObj.size(); j++) {
    ...
    // Si no estamo en el último factor agregamos el asterisco de los productos
    // y espacios
